...
> Cowork skill. Reference files (playbook/, knowledge/, scripts/) and writable data live under `C:\osi-claude-brain\`. Identity is hardcoded for Brian Charrette (see STEP -2). Install by dragging the .skill file into a Cowork chat.

# OSI Global, LinkedIn Prospect Qualification

---

## 🚨 STEP -2: IDENTITY (hardcoded for Brian Charrette)

This skill is configured for Brian Charrette at OSI Global. Identity values are hardcoded throughout this file. No identity-file lookup is needed; do not stop to look for one.
- HubSpot Owner ID: 213536174 (assigns every contact, note, and task to Brian)
- HubSpot Portal ID: 21878985 (builds the HubSpot record links and the Chrome enroll URL)
- From email: bc@osihardware.com
- Sign-off name: Brian
...
🛑 **Do NOT bulk-load tool schemas at session start.** Loading many MCP schemas in one ToolSearch trips an Anthropic API error, `400 tools.X.custom.input_schema: int too big to convert`, from an oversized integer in a ZoomInfo or Chrome tool schema. Once it is in the tool list, every message in the session fails. Load each GROUP on demand, with its own small ToolSearch, just before that group is first used. Each group loads once and stays live.

Groups, in usual order of first use. Load ONLY the group you are about to use, and NEVER combine ZoomInfo and Chrome in one ToolSearch:
1. **HubSpot core (at session start):**
   `ToolSearch({ query: "hubspot search_crm_objects manage_crm_objects get_crm_objects search_owners", max_results: 6 })`
   This matches your HubSpot MCP tools by name regardless of your connector's server id. Do NOT hardcode a server id here.
2. **Bash (before the first file or script op):**
   `ToolSearch({ query: "select:mcp__workspace__bash", max_results: 2 })`
3. **ZoomInfo: do NOT load the ZoomInfo MCP tools.** The ZoomInfo MCP is unstable (500s) and its tool schema is what trips the `tools.X int too big to convert` 400 that kills the session. Never ToolSearch the ZoomInfo MCP tools. Do ALL enrichment in the ZoomInfo web app in Chrome instead (navigate to app.zoominfo.com, read the contact panel for email + phone).
4. **LinkedIn / Chrome reading (just before the first browser action):**
   `ToolSearch({ query: "select:mcp__Claude_in_Chrome__navigate,mcp__Claude_in_Chrome__get_page_text,mcp__Claude_in_Chrome__tabs_context_mcp", max_results: 3 })`
   Do NOT load `browser_batch`. Its heavier schema is a 400 risk. navigate + get_page_text cover browsing and the ZoomInfo web app.
4b. **Chrome clicking for sequence enrollment (only when enrolling a Sniper CALLS contact, per sequence-enrollment.md):**
   `ToolSearch({ query: "select:mcp__Claude_in_Chrome__computer,mcp__Claude_in_Chrome__find", max_results: 3 })`
   Load these only when you reach a calls-only enrollment. They drive the HubSpot enroll dialog (click More, pick the sequence, Start sequence).
5. **Web search + task tools (just before first use):**
   `ToolSearch({ query: "select:WebSearch,TaskCreate,TaskUpdate,TaskList", max_results: 5 })`

Skip any group already loaded earlier in the conversation. If a single group ever still trips the 400, split it and load its tools one at a time to isolate and skip the offender.

---

## 🛑🛑 ZOOMINFO: USE THE WEB APP IN CHROME, NOT THE MCP (overrides everything below)

**There are TWO separate ZoomInfo credit pools. This is the whole point:**
- The **MCP / API** draws on the shared BULK pool, which is EXHAUSTED. Every MCP call fails (credit-limit / 500). On top of that, the ZoomInfo MCP tool schema crashes the entire session with `400 tools.X.custom.input_schema: int too big to convert` the moment it loads. The bulk credits are gone. Stop reaching for them.
- The **web app in Chrome** (app.zoominfo.com, Brian's personal login) draws on Brian's PERSONAL pool, which has about 1,000 credits. This is the ONLY ZoomInfo path that has credits, and it does not crash.

**So: do ALL ZoomInfo work in the web app in Chrome. Never touch the MCP.**
- NEVER ToolSearch, load, or call any ZoomInfo MCP tool (`search_contacts`, `enrich_contacts`, `enrich_news`, `enrich_scoops`). Not once. Its schema crashes the session with the `tools.X int too big` 400.
- Everywhere this skill says to run `search_contacts` (Step 1B discovery) or `enrich_contacts` / the 7-attempt retry matrix (enrichment), do that work in the **web app**: navigate to app.zoominfo.com in Chrome, search the company or person, read the contact panel for email + direct + mobile. This spends Brian's personal credits, which is what we want.
- Four-source discovery still applies: HubSpot (1A), ZoomInfo web app (1B), LinkedIn browse (1C), LinkedIn keywords (1D). Do 1B in the web app, never the MCP.

This banner overrides any instruction below that says to call a ZoomInfo MCP tool. Ignore it and the session dies on the next message.

---

## WHO OWNS WHAT

Strict boundary with **osi-outreach-sequence**. All enrollment + connection-request routing follows `C:\osi-claude-brain\playbook\sequence-enrollment.md` (read it).

| Responsibility | Owner |
|---|---|
| Verdict (Yes / No / Conditional) | this skill |
| Read LinkedIn profile in full | this skill |
| Resolve LinkedIn URL from name+company | this skill |
| ZoomInfo enrichment (email, direct phone, mobile) | this skill |
| Strategy note creation (includes the CONNECTION INVITE text) | this skill |
| No-email routing: enroll `Sniper CALLS` (has phone) or InMail tasks (no phone) per sequence-enrollment.md | this skill |
| Standalone connection task (calls-only and neither only) | this skill |
| Drafting 6 emails + writing AI fields | osi-outreach-sequence |
| Email-path enrollment (`Brian - AI` sequence, connection = step 1, start = Day 1) | osi-outreach-sequence |
| Day 1 stagger calc | osi-outreach-sequence |
| Excel tracker rows | osi-outreach-sequence |

**Handoff rule (route by email + phone, see sequence-enrollment.md for the full matrix):**
- **Yes WITH email** -> `HANDOFF: invoke osi-outreach-sequence on [First Last] at [Company].` Outreach drafts the emails, writes the AI fields, and enrolls the contact in the matched `Brian - AI` sequence (connection request is step 1 INSIDE the sequence; NO standalone connection task on the email paths).
- **Yes, NO email, HAS phone** -> do NOT hand off. Enroll in the matched `Sniper CALLS` sequence AND create the standalone connection task. Per sequence-enrollment.md.
- **Yes, NO email, NO phone** -> do NOT hand off. Standalone connection task + 2 LinkedIn InMail tasks. Per sequence-enrollment.md.

"Has phone" = a real direct or cell number, never the company switchboard / (HQ) line.
...
See `C:\osi-claude-brain\playbook\drafting-rules.md` Section 9 for verbatim phrasing and email-by-email behavior. The drafting skill (osi-outreach-sequence) loads drafting-rules.md at Step 0 and applies the rule at draft time. Qualification does NOT need to handle approved-vendor language -- just record the company name in the strategy note and the drafter handles the rest.

To add a company to the approved list: edit `C:\osi-claude-brain\approved-vendors.json` directly.
...
7. AND not on the hard-block list at `C:\osi-claude-brain\automation\hard-block.json`.
...
7. Generate the outreach package: strategy note with the CONNECTION INVITE line, call script, VM, LinkedIn invite, Email 1 opener. Annotate strategy note with `SOURCE: HubSpot shallow qualify (engagement-gated, last reply/meeting <date>)`.
8. Route the same as the deep path: Yes-with-email -> handoff to osi-outreach-sequence (drafts, writes AI fields, enrolls in the matched `Brian - AI` sequence with connection = step 1). No email -> Step 4 phone split. No standalone connection task on email paths.
...
3. On hard-block list (`C:\osi-claude-brain\automation\hard-block.json`): SKIP.
...
🛑 Reminder (see the ZoomInfo banner at the top): do this in the **ZoomInfo web app in Chrome** (personal credits), NOT the `search_contacts` MCP tool. The MCP hits the dead bulk pool and crashes the session. The instructions below describe the MCP for reference, but run the equivalent search in the web app.
...
- **NEVER print email bodies or subjects in chat.** The drafter writes them to the HubSpot AI fields, enrolls the contact, and reports ONE line: `Enrolled: [Name] | [Sequence]`.
...
After a company's Yes candidates are all processed, print a Markdown TABLE of everyone ACTUALLY enrolled (sequence enrollment verified, or a Sniper CALLS enrollment for the calls-only path). The LinkedIn and HubSpot columns MUST be live Markdown hyperlinks, not raw URLs. Use this exact structure:

```
[Company]: [N] Contacts Sequenced ✅

| # | Name | Sequence | Enroll By | LinkedIn | HubSpot |
|---|------|----------|-----------|----------|---------|
| 1 | [First Last] | [Sequence] | [Mon D] | [LinkedIn](hs_linkedin_url) | [HubSpot](https://app.hubspot.com/contacts/21878985/record/0-1/[contactId]) |
| 2 | ... | ... | ... | ... | ... |
```

Worked example (format reference only):
```
HU APL: 10 Contacts Sequenced ✅

| # | Name | Sequence | Enroll By | LinkedIn | HubSpot |
|---|------|----------|-----------|----------|---------|
| 1 | Michael Misumi | Pain-Led TPM | Jun 11 | [LinkedIn](https://www.linkedin.com/in/mmisumi) | [HubSpot](https://app.hubspot.com/contacts/21878985/record/0-1/12345) |
| 2 | Daniel Sorensen | Pain-Led TPM | Jun 17 | [LinkedIn](https://www.linkedin.com/in/dsorensen) | [HubSpot](https://app.hubspot.com/contacts/21878985/record/0-1/12346) |
| 3 | Andrew Liu | Pain-Led TPM | Jun 24 | No LinkedIn | [HubSpot](https://app.hubspot.com/contacts/21878985/record/0-1/12347) |
```

Then one count line below the table: `[Company]: X Yes, Y No, Z Conditional`.

Rules for the recap table:
- One row per person ACTUALLY enrolled (email paths and calls-only), numbered in enroll order. No-email-no-phone fallbacks (connection + InMail only) and No / Conditional verdicts are NOT in the table.
- **LinkedIn column:** `[LinkedIn](<hs_linkedin_url>)` using the contact's `hs_linkedin_url`. If blank, write the plain text `No LinkedIn` (no link).
- **HubSpot column:** ALWAYS `[HubSpot](https://app.hubspot.com/contacts/21878985/record/0-1/<contactId>)` built from portal id `21878985` and the contact id. Never leave it blank.
- **Sequence column:** the matched sequence name (Pain-Led TPM, Sample-Offer Network, Sample-Offer Server, Pain-Led Storage, Pain-Led DWDM, etc.).
- **Enroll By column:** the sequence start date (Day 1 from the stagger for email paths, or the calls-path default start). Format `Mon D`, e.g. `Jun 11`.
- Any `enroll-failed` person goes in a separate `NEEDS MANUAL ENROLL:` line BELOW the table so Brian can finish them by hand.
- This table IS the deliverable summary. No paragraphs, no email bodies (the minimal-output rule still applies). The only chat output at end of company is this table plus the one count line.
...
🛑 Reminder (see the ZoomInfo banner at the top): do all enrichment in the **ZoomInfo web app in Chrome** (Brian's personal credits, ~1,000). Do NOT call `enrich_contacts` / `search_contacts` MCP tools or run the 7-attempt retry matrix against the API. The MCP hits the dead bulk pool and crashes the session. The retry matrix below is reference for what to look up; perform it in the web app.
...
1. **hard-block.json.** Read `C:\osi-claude-brain\automation\hard-block.json`. If the address (or the contact) is listed, it is blocked. No tool call needed.
...
- Standalone connection task creates (Step 3, calls-only and no-email-no-phone paths only)
- Sniper CALLS / InMail task creates (Step 4)
...
**Why this rule exists:** 2026-05-06, Company Mode qualification of Instinet stopped mid-run to ask Brian to approve a contact update for Brian Banhidi (stale @baml.com -> @instinet.com). Brian had already said "go" at session start. The tool's confirmation step is a safety measure for ad-hoc use; it is not intended to interrupt a pre-approved qualification batch. The fix is to always use `CONFIRMATION_WAIVED_FOR_SESSION` so the tool never prompts.

Always create regardless of data:
- Strategy and Fit note (includes the CONNECTION INVITE line).

Connection request handling depends on the route (see Step 3 and `C:\osi-claude-brain\playbook\sequence-enrollment.md`):
- **Email paths:** NO standalone task. The connection request is step 1 inside the `Brian - AI` sequence, set at enrollment by osi-outreach-sequence.
- **Calls-only and no-email-no-phone:** create the standalone LINKED_IN_CONNECT task here.

Email creation, scheduling, the 6-email sequence, and email-path enrollment are owned by `osi-outreach-sequence`, NOT here.

### HARDWIRED RULE -- STAGGER IS OUTREACH-SEQUENCE'S JOB

This skill does NOT compute the Email 1 Day 1 for email paths. `osi-outreach-sequence` owns the stagger table, computes the real Day 1, and sets it as the sequence start date at enrollment. One skill owns stagger. No drift.
...
- "Create a standalone connection task on the email paths." NO. The connection request is step 1 inside the sequence on email paths.

### HARDWIRED RULE -- ATOMIC WRITES TO STATE FILES

Every modification to `C:\osi-claude-brain\overnight-candidates.json` and `C:\osi-claude-brain\automation\email-queue.json` is an atomic write. Read the full file, modify in memory, write to `<file>.tmp`, then `os.replace(tmp, file)`. NEVER `open(file, 'w')` directly.

```python
import os, json
PATH = 'C:/osi-claude-brain/overnight-candidates.json'
...
Step 1, Step 2, then the route-specific Step 3 / Step 4 execute as one transaction. Fixed order. If any step fails or its read-back fails, abort BEFORE handing off to osi-outreach-sequence. Flip candidate to status `pending-relookup`, log to `overnight-run-log.md`.

Order:
1. **Step 1** -- Update or create contact.
2. **Step 2** -- Create Strategy and Fit note with the CONNECTION INVITE line (depends on Step 1 contact ID).
3. **Route (Step 3 / Step 4, per sequence-enrollment.md):**
   - **Yes WITH email** -> HANDOFF to osi-outreach-sequence. It drafts, writes AI fields, and enrolls in the matched `Brian - AI` sequence (connection request = step 1 inside the sequence). No standalone task here.
   - **Yes, NO email, HAS phone** -> enroll in the matched `Sniper CALLS` sequence (verify the enroll) + create the standalone connection task (read it back).
   - **Yes, NO email, NO phone** -> create the standalone connection task (read it back) + 2 InMail tasks.
4. **Step 3.5** -- where a standalone connection task was created, read it back: confirm `hs_task_type` is `LINKED_IN_CONNECT`, associated to the contact. Where a sequence was enrolled, confirm the "Successfully enrolled" banner. If any check fails: ABORT, flip to `pending-relookup`, log.

**FORBIDDEN paths:**
- "Create a standalone connection task on the email paths." NO. Connection is step 1 inside the sequence there.
- "Skip the read-back / enroll verification, the call returned." NO. The ID or call returning does not prove the write is correct.
- "If verification fails, retry once and continue." NO. Failure means something is structurally wrong. Flip to `pending-relookup` (task) or log `enroll-failed` and flag the person (enrollment).
...
Read `C:\osi-claude-brain\playbook\hubspot-data-quality.md` for required fields, phone format, mobile rule, timezone bucket, pre-write checklist.
...
OPENER: [full opener from C:\osi-claude-brain\playbook\opener-library.md]
VM: [one line, 15s max. One-sentence hook. "I'm sending you something right now, subject line is [Email 1 subject]." Ends with "that's b c at osi hardware dot com." Present/future tense only.]
...
CONNECTION INVITE
[The actual LinkedIn connection-request message, under 300 chars, references the Personal Hook, no pitch, no OSI mention, no sign-off. This is typed into the sequence step-1 connection task note (email paths) or the standalone connection task note (calls-only / neither) at enrollment. See C:\osi-claude-brain\playbook\sequence-enrollment.md.]
...
The SEQUENCE line drives the auto-enrollment routing (which `Brian - AI` or `Sniper CALLS` sequence the contact lands in). See `C:\osi-claude-brain\playbook\sequence-enrollment.md`.
...
### Step 3: Connection request + enrollment routing (per C:\osi-claude-brain\playbook\sequence-enrollment.md)

Routed by email + phone. Read `C:\osi-claude-brain\playbook\sequence-enrollment.md` for the full matrix and the Chrome enroll procedure.

- **Yes WITH email:** do NOT create a standalone connection task here. The connection request is step 1 INSIDE the `Brian - AI` sequence; osi-outreach-sequence types the CONNECTION INVITE text into that step-1 task note at enrollment. This skill's job ends at: contact (Step 1) + strategy note with the CONNECTION INVITE line (Step 2), then HANDOFF.
- **Yes, NO email, HAS phone:** enroll in the matched `Sniper CALLS` sequence (sequence-enrollment.md) AND create the standalone connection task below.
- **Yes, NO email, NO phone:** create the standalone connection task below + the 2 LinkedIn InMail tasks (Step 4).

**Standalone connection task (calls-only and neither only):**
...
- Due: next business day at 4 PM ET (calls-only default, no stagger)
- Notes: the CONNECTION INVITE text from the strategy note. Raw message only.
- Owner: 213536174.

**CONNECTION INVITE quality bar (wherever the invite is used, in-sequence note or standalone task):**
- [ ] The actual message Brian sends, not an instruction or "lead with X angle."
- [ ] References the Personal Hook. No pitch, no OSI mention, no product names.
- [ ] Under 300 characters. No sign-off, no "Brian."

CORRECT: `Tim, 25 years running NFL IT infrastructure is a long time to see every generation of optical gear come and go. I'd love to connect.`
WRONG: "Send connection request, lead with the tenure angle." (instruction, not a message). WRONG: any pitch or OSI mention.

### Step 3.5: Verification (MANDATORY)

- **Standalone connection task (calls-only / neither):** fetch it back by `hs_object_id`. Confirm it exists, `hs_task_type` = `LINKED_IN_CONNECT`, associated to the contact. Fail -> ABORT, flip `pending-relookup`, log.
- **Sequence enrollment (email paths via outreach, or `Sniper CALLS` here):** verification is the "Successfully enrolled" confirmation in the Chrome procedure (sequence-enrollment.md). No confirmation -> log `enroll-failed`, flag the person, do not mark done.

### Step 4: NO email routing (per C:\osi-claude-brain\playbook\sequence-enrollment.md)

**Trigger:** no usable email after the full ZoomInfo retry matrix (or the personal-email / blocked-address check forced no-email).

Route by phone (direct or cell, NEVER the switchboard):
- **HAS phone:** enroll in the matched `Sniper CALLS` sequence (Network / TPM / DWDM / S/S, by the strategy note SEQUENCE line) via the Chrome procedure, default start, no stagger. PLUS the standalone connection task (Step 3). No InMail. The call task notes carry the LIVE CALL SCRIPT + VM from the strategy note.
- **NO phone:** standalone connection task (Step 3) + 2 LinkedIn InMail tasks:
  - Duplicate check: if a NOT_STARTED / IN_PROGRESS `LINKED_IN_MESSAGE` task exists, skip both.
  - Task 1: `LINKED_IN_MESSAGE`, "1st LI -- [First Last] | [Company]", due 7 days. Note: 1st LI message (3 sentences max).
  - Task 2: `LINKED_IN_MESSAGE`, "2nd LI -- [First Last] | [Company]", due 21 days. Note: 2nd LI message (1-2 sentences).

Do NOT hand off to osi-outreach-sequence on either no-email path.
...
- **`C:\osi-claude-brain\playbook\product-lines.md`** -- OSI product lines, sequence-type table, DWDM talking points, who buys what.
- **`C:\osi-claude-brain\playbook\vertical-intel.md`** -- what to lead with by industry. Park Place / Service Express merger wedge. TPM positioning.
- **`C:\osi-claude-brain\playbook\opener-library.md`** -- 12 cold-call openers + cold call rules.
- **`C:\osi-claude-brain\playbook\pain-and-objections.md`** -- pain points + discovery questions by product line, objection-handler bank, secondary-source positioning. Use when writing the Live Call Script (objection handling) and the Personal Hook / The Play.
- **`C:\osi-claude-brain\playbook\hubspot-data-quality.md`** -- required fields, phone format, timezone buckets, pre-write checklist.
- **`C:\osi-claude-brain\playbook\voice-rules.md`** -- Brian's voice + humanization filter. Apply to call script, VM, LinkedIn invite text.
...
**Target Sequences** -- every applicable OSI product line. Choose from C:\osi-claude-brain\playbook\product-lines.md sequence list.
...
OPENER: [full opener from C:\osi-claude-brain\playbook\opener-library.md, or custom if HOOK is strong]
```

### 3. Voicemail

15s max. One voicemail, never two. Hook drawn from Personal Hook. Name Email 1 subject. End with "that's b c at osi hardware dot com." No phone number. Present/future tense only.

```
"Hey [Name], Brian with OSI Global. [One-sentence hook]. I'm sending you something right now, subject line is [Email 1 subject]. That's b c at osi hardware dot com."
```
...
For every Yes with valid email (after the Step 2 strategy note is live), end with:

> HANDOFF: invoke osi-outreach-sequence on [First Last] at [Company]. Strategy note live on HubSpot contact ID [id]. Personal Hook: [full hook text verbatim from THE PERSONAL HOOK section -- do NOT truncate or summarize]. Fresh hook: [full one-line summary + URL, or "none"]. CONNECTION INVITE: [verbatim invite text from the strategy note]. Recommended sequence: [Sample-Offer Network / Sample-Offer Server / Pain-Led TPM / Pain-Led DWDM / Pain-Led Storage / Pain-Led Pre-Owned]. Email: [verified email address]. Phone present: [yes direct/cell | no].

The handoff Personal Hook and CONNECTION INVITE must be the complete text written to the strategy note, not a paraphrase. osi-outreach-sequence uses them verbatim to draft Email 1 and to populate the step-1 connection task at enrollment WITHOUT re-reading the strategy note or LinkedIn profile.

Outreach-sequence owns stagger computation. It computes the real Day 1, drafts all 6 emails, writes the AI fields, and enrolls the contact in the matched `Brian - AI` sequence (connection request = step 1, start = Day 1) via Chrome.

If ZI retry matrix returned no email after all 7 attempts: do NOT hand off. Route by phone per Step 4 (Sniper CALLS + connection task, or connection task + 2 InMail).
...
Read `C:\osi-claude-brain\automation\email-queue.json`. Extract every unique `company` value (case-insensitive). This is your exclusion list -- any company already in the queue has active sequences running and must NOT be presented as a cold target.

Also read the "Companies Prospected" tab of `C:\osi-claude-brain\prospects-tracker-new.xlsx` for any additional historically prospected companies.

**DNP filter:** Read `C:\osi-claude-brain\do-not-auto-prospect.json`. Any company whose name case-insensitively matches (substring or exact) a name in that file is permanently excluded from Auto Mode. This list covers companies Brian does not want surfaced by automation -- they can still be manually prospected if Brian names them explicitly, but they will never appear in an auto-generated shortlist. Add these to the exclusion set alongside the queue companies.

After building the exclusion list:
- Update the "Companies Prospected" tab in `C:\osi-claude-brain\prospects-tracker-new.xlsx` with any companies from the queue that are not already listed there (company name + last sequence date, sorted by date descending).
...
wb = openpyxl.load_workbook(r'C:\osi-claude-brain\prospects-tracker-new.xlsx')
...
for row in ws.iter_rows(min_row=2):
    if str(row[0].value).strip().lower() == company_name.strip().lower():
        ...
        break
wb.save(r'C:\osi-claude-brain\prospects-tracker-new.xlsx')
```
...
5. **Route the handoff by task subject** (one qualified Yes-with-email contact at a time -- IMMEDIATE HANDOFF, NO BATCHING, same rule as Company Mode):
   ...
   The outreach skill owns stagger math, drafting, AI-field writes, and the Chrome sequence enrollment (connection request = step 1 inside the sequence). Do not draft emails here. Before any handoff, the Blocked address check (see "Blocked address check" section above) must pass. A blocked address takes the no-email disposition in step 6.

6. **No email after the full ZI matrix, OR a blocked address:** do NOT hand off. Route by phone per Step 4: HAS phone -> enroll in the matched `Sniper CALLS` sequence + standalone connection task; NO phone -> standalone connection task + 2 InMail tasks. Mark the source enroll task COMPLETED with note "No email -- calls/LinkedIn fallback created" (or "Blocked address -- calls/LinkedIn fallback created").
...
8. **Final summary** to Brian:
   ...
   - No email, has phone -- enrolled Sniper CALLS + connection task: [N] -- names
   - No email, no phone -- connection task + 2 InMail: [N] -- names
   - NEEDS MANUAL ENROLL (enroll-failed): [N] -- names + reason
...
Append all Yes and Conditional to `C:/osi-claude-brain/prospects-tracker-new.xlsx` (Prospects tab).
...
- ZoomInfo retry matrix returned no FULL_MATCH after all 7 attempts -> `yes-no-email`, route by phone per Step 4 (Sniper CALLS + connection task, or connection task + 2 InMail), NO handoff. Log all 7 attempts in strategy note.
...
Every failure logs to `C:/osi-claude-brain/overnight-run-log.md`. Never silent.
...
- Standalone connection tasks (calls-only / no-email-no-phone) -> ALWAYS `LINKED_IN_CONNECT`. Never `LINKED_IN_MESSAGE`. Never `TODO`. Email paths get NO standalone task (connection = step 1 inside the sequence).
- Connection request routing + enrollment follow `C:\osi-claude-brain\playbook\sequence-enrollment.md`. Enroll per contact immediately, verify each, never batched.
- Stagger calculation is outreach-sequence's job. This skill does NOT read `state.stagger` or compute Day 1.
- Step 3.5 verification (task read-back, or "Successfully enrolled" banner) is MANDATORY. Failed -> `pending-relookup` (task) or `enroll-failed` + flag (enrollment).
- Timezone: 6-bucket only. See C:\osi-claude-brain\playbook\hubspot-data-quality.md.