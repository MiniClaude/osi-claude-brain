...
This skill is invoked from the orchestrator inline. "Invoked" means the orchestrator reads this SKILL.md into its own context and runs Profile Mode there. It does NOT mean the orchestrator dispatches an `Agent` subagent to run this skill. If a future Claude session ever uses the Agent tool to run this skill on a scheduled fire, that is a bug, every Agent dispatch spawns a fresh sandbox that re-prompts Andy for folder mount approval, which violates the no-prompt rule below. Run inline. See osi-overnight-runner SKILL.md for the longer explanation. **Why this rule exists:** 2026-04-29 fire dispatched 8 subagents and triggered 8 mount prompts before Andy halted the run.

---

## 🛑 HARDWIRED RULE, NO APPROVAL PROMPTS DURING SCHEDULED FIRES

If any tool call triggers an approval prompt of any kind during a scheduled-task fire (overnight runner, processing recurring, monitor, email-sender, any cron-fired session), abort that step immediately. Do not proceed. Do not retry. Do not pivot to "another way to verify the same fact." Mark the candidate **Conditional** with reason "could not verify employer without prompting Andy", log to `Claude-Brain/overnight-run-log.md`, advance to next candidate.

What this means in practice during scheduled fires:
- LinkedIn (Chrome MCP) is the ONE allowed verification source. No corporate website fetches, no Google for bio confirmation, no podcast pages, no press release fetches, no `web_fetch` to any non-LinkedIn domain.
- If LinkedIn 404s, restricts the profile, or otherwise blocks Path A: Conditional. Do NOT fall through to Path B.
- The "Mandatory fallback: Google via WebSearch" line in CONTACT VERIFICATION PROTOCOL is suspended for scheduled fires. Interactive sessions (Andy at keyboard) keep the fallback.
- ZoomInfo `enrich_scoops` and `enrich_news` are allowed (already-approved MCP). Other research moves to interactive only.

Interactive sessions (Andy at keyboard) retain Path B. The rule fires only when the session is launched by a scheduled task. Detect the session type from the dispatching context: any fire that arrives from osi-overnight-runner or any `<scheduled-task>` wrapper is non-interactive.

**Why this rule exists:** 2026-04-29, Patti Paulo (ExteNet Systems) had a restricted LinkedIn profile. The Path B fallback fetched `extenet.com/about-us/leadership/patti-paulo` to confirm employer. That fetch prompted Andy for browser permission overnight. The orchestrator promised Andy zero prompts during scheduled fires; this skill broke the promise via a documented fallback. Inverted the rule: any tool that prompts = abort, regardless of what the call is or which fallback authorized it.
...
This rule moved. See `C:\Claude-Brain\playbook\drafting-rules.md` Section 9 for the verbatim phrasing and email-by-email behavior. The drafting skill (osi-outreach-sequence) loads drafting-rules.md at Step 0 and applies the rule at draft time. Qualification does NOT need to handle approved-vendor language; it just records the company name in the strategy note and the drafter handles the rest.
...
🚨 **HARD GATE (added 2026-04-30 after Andy review):** This path is ONLY available when there is a recent two-way engagement signal proving the contact is currently reachable at the listed company. Mere HubSpot record presence is NOT sufficient. Title alone is NEVER sufficient. Without a recent reply or meeting, fall back to Path A (deep LinkedIn read) per CLAUDE.md hardline rule.

**Why this gate exists:** 2026-04-26, an earlier version of this path triggered on "HubSpot record + JAM-owned" alone. By 2026-04-26 it had produced 138 emails queued to 23 prospects across S&P Global, OEC Fiber, Fidelity Communications, Vero Networks, and Midcontinent without anyone confirming those people were still employed there. The path was ripped out the same day. This rewrite (2026-04-30) preserves the speed-up for known-active contacts while making the trap impossible: you cannot reach this path without a real signal that the email currently belongs to someone still at the target company.
...
4. Email domain matches the company's known/derived primary domain (no jane.doe@gmail.com on a Big River prospect), AND
5. **Recent positive engagement signal: at least ONE of:**
...
6. AND no hard-bounce ever recorded against this email (check `hs_email_bounce` and the most recent send's status), AND
7. AND not on the hard-block list at `C:\Claude-Brain\hard-block.json`.

**Why 12 months and not 24:** median B2B tenure for senior IC / director roles is 3-4 years. Reply within 12 months + no bounce since = ~85-90% still at company. Reply within 24 months + no bounce = ~70-75%. The 70s is too loose for "skip verification entirely." Auto-forward windows after departure are typically 90 days, which sit comfortably inside a 12-month gate. Andy chose 12mo over 24mo on 2026-04-30 review.

If ANY of 1-7 fails → fall back to Path A (deep LinkedIn read). No exceptions. **Opens, deliveries, clicks, and "contact has activity" without a reply or meeting do NOT qualify** — those are weak signals. Replies and meetings prove two-way contact with a human at the company.

### How to verify the engagement gate (Step 0 of Shallow Qualify)

Before doing ANY other work in this path, check engagement signals via `get_crm_objects` and the contact's associations. Read:
- `hs_email_last_reply_date` if populated, OR enumerate associated emails where direction is `INCOMING_EMAIL` and pick max `hs_timedelta` within 12mo.
- Associated meetings (`meetings` association), filter to `hs_meeting_outcome != NO_SHOW` and `hs_timestamp` within 12mo.
- `hs_email_bounce` and most recent outbound email status. If any hard-bounce ever, gate fails.

If engagement gate passes → continue to Step 1. If it fails → log `engagement_gate_failed: <reason>` in the strategy note, escalate to Path A (deep LinkedIn read), do NOT shortcut.

### Shallow Qualify steps (only after Step 0 gate passes)
...
6. **Personal Hook**, thinner without deep read. Build from: the recent reply/meeting itself (best source — quote what they said last), previous employers (HubSpot notes), company news (`enrich_scoops` or `enrich_news` one quick call), or vertical-specific hook fallback.

7. **Generate outreach package**, strategy note, LINKED_IN_CONNECT task, call script, VM, LinkedIn invite, Email 1 opener with Personal Hook. Annotate strategy note with `SOURCE: HubSpot shallow qualify (engagement-gated, last reply/meeting <date>)`.

8. **Handoff** to osi-outreach-sequence, same format as deep path. Email 1 Day 1 transactional rules below apply.

### When to escalate back to deep (even if Step 0 gate passed)
- Vague title ("IT Specialist", "Technology Manager" with no industry clue)
- Most recent reply explicitly mentioned departure ("just heads up, I'm leaving in two weeks", "after my last day", "transitioning out") — read the most recent reply's body if accessible and look for departure language. If found, treat as ❌ disqualifier, NOT a fast-path candidate.
- HubSpot company field disagrees with the email domain in a way the gate didn't catch (rare but possible)
...
Every modification to `C:\Claude-Brain\overnight-candidates.json` and `C:\Claude-Brain\email-queue.json` is an atomic write. Read the full file, modify in memory, write the modified content to `<file>.tmp`, then `os.replace(tmp, file)`. NEVER `open(file, 'w')` directly. A mid-write crash on the live file leaves it truncated and the next reader fails to parse JSON. We hit this corruption twice on 2026-04-29: once on overnight-candidates.json (Michael Bahr entry mid-write), once mid-stagger-update. Both ate hours of Andy's time. No exceptions. This rule applies to every status flip (pending → conditional, pending → no, pending → yes-with-email, pending → pending-relookup, etc.) and every metadata update.
...
If you find yourself about to write directly to the live file, STOP. Use the atomic pattern above. There is no acceptable shortcut.

### 🛑 HARDWIRED RULE, STEPS 1-3.5 ARE TRANSACTIONAL

Step 1, Step 2, Step 3, Step 3.5 below execute as one transaction. The order is fixed. If any step fails or its read-back fails, the skill aborts BEFORE handing off to `osi-outreach-sequence`. The candidate is flipped to status `pending-relookup` in `state.candidates` (atomic write per the rule above) and a flag is logged to `overnight-run-log.md`. NO email queue entries are written for failed transactions. Andy reviews `pending-relookup` candidates manually next session.
...
- Earnings beat or miss (use carefully, only when it connects to infrastructure spend)
- Product launch
...
If no strong fresh hook found, the strategy note's `Fresh hook` line reads exactly: `Fresh hook (30-day news): none`.

Do NOT write filler. Do NOT include weak hooks because something has to go in that field. The drafter (`osi-outreach-sequence`) consumes this field and will not reference a missing hook. Filler in this field gets surfaced into Email 1 as bad personalization.

If a strong hook IS found, record in Strategy and Fit:
`Fresh hook (30-day news): [one-line summary + source URL]`

One search. No rabbit holes. No second-pass searches to find a better hook. If the first search returns weak signals only, the field is `none`.
...
**The Personal Hook**, 1-2 specific LinkedIn details. Priority order: (1) recent post/repost/comment in last 3-6 months [strongest], (2) recent job change / cert, (3) past company that's an OSI customer, (4) specific named project, (5) unusual skill combo. Hook appears in Email 1 + LinkedIn invite.

### 🛑 PERSONAL HOOK QUALITY GATE, hardwired

The hook MUST be one of the 5 priority types above. The following are NOT Personal Hooks and the strategy note CANNOT be saved with them:
...
- Company size or industry alone. "Big bank" or "large carrier" is not a hook.
- Generic compliments. "Impressive background" is not a hook.

If after a full LinkedIn read (About + Experience + Skills + Activity feed in `/recent-activity/all/`) NO qualifying hook of priority types 1 through 5 can be pulled, the verdict is downgraded to ⚠️ Conditional with reason "no Personal Hook available". The candidate is flipped to `pending-needs-hook` in `state.candidates`. Andy reviews next session and either pulls a real hook manually or marks the candidate `no-hook-available`.

The drafter (`osi-outreach-sequence`) refuses to write Email 1 with a thin or generic hook. Filling the hook gate here at qualification time is the ONLY way to keep the drafter from blocking later.

**Why this rule exists:** the 2026-04-30 Christopher Lawrence email shipped with "BNY's Pittsburgh infrastructure footprint is significant" as the ostensible Personal Hook. That's not a hook. Email 1 read like a generic dump because the strategy note didn't carry a real hook in the first place. The fix lives at strategy-note-write time, not at email-draft time. Stop the bad hook from being written into the note in the first place.
