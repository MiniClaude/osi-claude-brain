...
description: >
  ...
  any non-OOO reply (including shipment address confirmations), flags anything needing Brian's
...
Check everything running, catch anything broken, fix it automatically, tell Brian what you did. Output a clean summary. Fast.
...
**Any time Email 1's send date changes for any reason, stranded fix, overdue fix, ordering fix, Brian request, any reason at all, the HubSpot "Sales Nav -- Send connection request" task for that prospect MUST be rescheduled to match the new Email 1 date. This is not optional. This is not a secondary step. Do it in the same write pass.**
...
5. Zero or multiple NOT_STARTED matches: do NOT auto-update. Flag for Brian to handle manually. Still fix the email queue.
...
1. **New send date AND send time:** determine the new date and time together.
   ...
   - If current ET time is 15:30 or later (only the 4pm window remains and there is not enough time for Brian to act on pre-flight risks) → reschedule to the **next business day**. Keep the entry's original `sendTime` (it will hit its normal window the next day).
...
4. If no HubSpot contact found for the bounced address, flag it for Brian to investigate manually
...
If the reply is not an OOO, treat it as human engagement and auto-pause the remaining queue entries for that prospect immediately. No waiting for Brian's confirmation. No lost momentum while Brian is asleep.
...
4. Update the HubSpot strategy note on the contact, prepend this line:
   > REPLY RECEIVED [date], sequence auto-paused. Brian to review and decide whether to resume or cancel.
...
Fill in everything known now. Leave Date Shipped, Tracking #, and Follow-up Status blank for Brian to complete.
...
When Brian says "resume [name]": flip those entries back to `"pending"` and adjust send dates if needed so the next send is at least 2 business days out from today. **Atomic write only** (`.tmp` + `os.replace`), never direct.

When Brian says "cancel [name]": change to `"canceled-reply"` and leave them. **Atomic write only**.
...
Do not ask Brian what to do. Auto-reschedule immediately.
...
- If zero matches or multiple NOT_STARTED matches: do NOT auto-update. Flag for Brian to handle manually.
...
3. Three cases:
   ...
   **Case C, No task found:** Create the task now. Fields:
   ...
   - Due date: today
   - Type: `TODO`
   - Owner: Brian (HubSpot owner ID 213536174)
...
This runs every day before the pre-flight risk check. No E1 should go out without Brian knowing whether the connection request has been sent.

---

## Step 5.5: Pre-Flight Risks (run before listing today's sends)

For every queue entry firing TODAY, run a series of checks against HubSpot signal at the same company. Anything flagged is surfaced in the PRE-FLIGHT RISKS section of the summary so Brian can swap or hold before the 4 PM batch.
...
- If the entry has `emailResolution: "dominant-pattern"` or `"manual-required"` → flag. Brian verifies before send.
...
Search HubSpot for any contact whose `company` matches AND `hs_email_last_reply_date >= today - 7 days`. If yes → flag: someone at the company is engaged. Brian may want to coordinate or hold.
...
Verification is per-prospect, not per-company. Never blanket-flag everyone at the 5 unverified-batch companies, that wastes Brian's time on prospects he's already cleared.
...
This way Brian verifies once per prospect (manual LinkedIn check or qualification skill writes the strategy-note line) and never sees that prospect flagged again.

**Check 6, `hs_additional_emails` mismatch.**
For each prospect, pull the contact's `hs_additional_emails`. If the queue entry's `to:` is in that list (i.e. we're sending to a known alt) → flag. Brian may have intended the primary.
...
If `findings` is non-empty, surface a `WEEKLY VALIDATOR AUDIT` section in the daily summary listing each violating entry. This is informational only, the emails already shipped. The point is to show Brian the drift so he can fix the upstream skill that produced the bad body.
...
This gives Brian a heads-up on what's going out today so he can make his calls before 3 PM.
...
- Manual cancellations Brian requested ("cancel sequence for [name]")
- Reply-based cancellations Brian confirmed
...
- Replied, Brian canceled
- Connected, Brian canceled
- Brian request
...
- Never modify tasks except to cancel on a confirmed hard bounce, auto-pause on a non-OOO reply, or Brian's explicit instruction.
- Never flag out-of-office auto-replies as actionable. Never pause on OOO.
- Auto-pause (not cancel) on non-OOO replies. Brian decides whether to resume or cancel after reviewing.
- **🚨 SALES NAV RULE, see the prominent callout at the top of this skill. Any time E1's date changes for any reason, the HubSpot Sales Nav connection request task moves with it. No exceptions. No "I'll flag it for Brian." Fix it in the same pass.**
...
- Keep the summary under 30 lines if possible. Brian reads this at 11 AM and needs to act on it fast.
