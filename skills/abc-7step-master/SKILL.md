...
Open `C:\Users\MINI OSI RIG\OneDrive - OSI Hardware\Documents\Claude\OSI-Brain\email-queue.json` using the OneDrive-safe Python pattern (try local `open(path, 'r')`, fall back to SharePoint MCP on EINVAL — see Queue Write Pattern below).
