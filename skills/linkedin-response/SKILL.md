...
## MANDATORY: Browser Tab Cleanup (run at the end of EVERY job)

This skill opens Chrome tabs. Cleanup is a required final step on every run — success, failure, or early stop:

1. Track the ID of every tab this run opens (`tabs_create`, `navigate` in a new tab, Outlook compose windows, LinkedIn/Sales Nav pages, etc.).
2. When the job completes, close ALL of them with `mcp__Claude_in_Chrome__tabs_close_mcp`.
3. NEVER close tabs that were already open before the run started — only tabs this run created.
4. One exception: if a tab is intentionally left for Brian to act on (e.g., a pre-composed Outlook email waiting for him to click Send), leave that single tab open and tell him it was left open on purpose.
5. End the final summary with a confirmation line: "Tab cleanup: closed N tabs opened during this run."
