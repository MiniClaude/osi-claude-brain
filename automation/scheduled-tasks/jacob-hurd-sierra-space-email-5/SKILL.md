---
name: jacob-hurd-sierra-space-email-5
description: Email 5 to Jacob Hurd (Sierra Space) — "Any thoughts?" reply — May 25 @ 9:05 AM PT
---

Send a reply to Jacob Hurd at Sierra Space using Outlook. This must be a REPLY in the original thread, not a new email.

1. Navigate to https://outlook.office.com in Chrome
2. If a login screen appears, STOP and notify Brian via a notification — do not attempt to send the email. Also create a todo in the task list titled "Outlook session expired — resend Jacob Hurd Email 5" so Brian can recover manually.
3. Go to Sent Items
4. Find the oldest email sent to jacob.hurd@sierraspace.com — that is the original cold outreach (subject: "On-prem costs after the $550M raise")
