---
name: ty-scottino-jackhenry-email-3
description: Email 3 to Ty Scottino at Jack Henry — soft touch
---
...
2. If a login screen appears, STOP and notify Brian via a notification — do not attempt to send the email. Also create a todo in the task list titled "Outlook session expired — resend Ty Scottino Email 3" so Brian can recover manually.
3. Click New mail
...
6. Click the Subject field and enter exactly: I've been ignored by better people (just checking in)
...
Just checking in one more time. At companies running financial services infrastructure at scale, the optics and TPM conversation usually comes up during a gear refresh or contract renewal -- not in between.

If that's not where you are right now, I get it. Reach out when the timing works. If you'd rather I stop following up, just say the word.
