---
name: bc-osi-outreach-sequence-v2
description: >
  ...
  Email 6 at 3 PM -- each email has its own dedicated window). LinkedIn connection request on
  Day 1. Runs interactively (Brian reviews before sending) or overnight (fully automated).
  Triggers on: "run a sequence", "outreach sequence", "build a sequence for", "run sequences
  for the following companies", pasting a LinkedIn profile, or uploading a profile file.
  Company Mode: finds and qualifies prospects at named companies. Auto Mode: pulls cold HubSpot
  companies with no activity in 6+ months. Always use this skill for new prospect outreach.
...
This skill works in tandem with **bc-prospect-qualification**. The boundary between them is strict. Never cross it.

| Responsibility | Owner skill |
|---|---|
| Qualify (verdict Yes / No / Conditional) | bc-prospect-qualification |
| Read the LinkedIn profile in full (About, Experience, Skills, activity / posts) | bc-prospect-qualification |
| ZoomInfo enrichment (email, direct phone, mobile) | bc-prospect-qualification |
| Strategy note on HubSpot contact (keywords, call script, VM, The Play, Personal Hook, ENROLL IN CALLS SEQUENCE label) | bc-prospect-qualification |
| `LINKED_IN_CONNECT` task creation (subject, type, owner, LinkedIn invite text in notes, provisional due_date = next business day) | bc-prospect-qualification |
| `LINKED_IN_CONNECT` task **final due_date** (updated to match Email 1's Day 1 after same-company stagger math) | **bc-osi-outreach-sequence-v2** (this skill) |
| No-email-no-phone LinkedIn message fallback tasks (1st LI, 2nd LI) | bc-prospect-qualification |
| Drafting the 6 emails | **bc-osi-outreach-sequence-v2** (this skill) |
| email-queue.json writes and scheduling | **bc-osi-outreach-sequence-v2** (this skill) |
| Same-company stagger math | **bc-osi-outreach-sequence-v2** (this skill) |
| Active sequence check (prevent duplicate enrollment) | **bc-osi-outreach-sequence-v2** (this skill) |
| Excel tracker Tab 1 (Prospects) and Tab 2 (Company Status) | **bc-osi-outreach-sequence-v2** (this skill) |
...
## 📥 HANDOFF FROM bc-prospect-qualification — assumes qualification has already run

When this skill runs, it assumes bc-prospect-qualification has ALREADY completed on this prospect, which means:
...
If the strategy note is missing on the contact (or this skill was triggered directly without qualification running first), STOP and invoke bc-prospect-qualification Profile Mode first. Then continue here.

When running in Company Mode (Brian provides company names) or Auto Mode (cold HubSpot companies picked by Claude): the first step is always to invoke qualification's Company Mode per company. Qualification returns a ranked shortlist. This skill then runs per ✅ Yes prospect (with email found) in that list.

---

## Your job

Brian has given you a LinkedIn profile. Produce the full outreach package: 6 personalized emails sent automatically via Outlook (each email has its own dedicated send window — Email 1 at 4 PM ET, Email 2 at 11 AM, Email 3 at 12 PM, Email 4 at 1 PM, Email 5 at 2 PM, Email 6 at 3 PM), 7 call tasks in HubSpot, and a LinkedIn connection request task on Day 1. Brian calls and sends the invite in the afternoon. Email 1 fires at 4 PM Eastern. Everything else runs itself.
...
This skill has three modes. All three assume bc-prospect-qualification runs on each candidate per the HANDOFF block above. This skill orchestrates WHEN qualification and outreach work happen, across time and across multiple scheduled sessions. Qualification's content (profile reads, verdict, note, LINKED_IN_CONNECT task creation) is NOT duplicated here; it lives in bc-prospect-qualification.

### Interactive Mode — Brian at the keyboard, single or small batch

**Triggered by:** Brian pastes a single LinkedIn profile, or says "build a sequence for [Name]", or a short multi-prospect prompt where Brian is present.

1. Invoke bc-prospect-qualification Profile Mode on the prospect. Qualification produces verdict. If No or Conditional: STOP-GATE applies, end here.
...
4. Present everything to Brian for review.
5. Brian says "ready."
6. Open Outlook in Chrome with Email 1 pre-composed. Brian clicks Send himself.
7. Brian says "sent."
...
**Batch size in Interactive Mode is flexible.** Brian can run 4-5 prospects in one session when he is watching.

### Company Mode — Brian names companies, runs overnight or unattended

**Triggered by:** "run sequences for the following companies: X, Y, Z" or similar multi-company prompts. Typically scheduled overnight while Brian sleeps.

Company Mode splits work across two phases:

#### PHASE 1 — Kickoff (Brian at the keyboard, roughly 10-20 minutes)
...
Decision tree based on leftover count and Brian's command:
...
- **Leftover count is between 1 and (batch count × batch size, default 21):** combine. Leftover entries stay at the front of the queue. Fill the remaining slot capacity with new candidates.
  - If Brian named companies: run steps 1-4 below on those companies and append their candidates to the back of the queue.
  - If Brian did NOT name companies (Auto Mode trigger): run the Auto Mode selector (cold companies + active-client filter + OSI fit check + queue-prevent filter, per the Auto Mode section), pick enough companies to cover the remaining gap, and append their candidates.
  - Goal: every overnight batch has work.

- **Leftover count is >= (batch count × batch size):** do NOT search for more. The pending queue already saturates tonight's capacity. Skip steps 1-4 entirely and go straight to the pre-schedule step. Tell Brian: "Leftover queue from [date] has N pending candidates, enough to fill tonight's 7 batches. No new search performed."

Never reset or archive the queue automatically. The only way to start entirely fresh is an explicit Brian command: "fresh run" or "reset queue and run overnight sequences for [companies]." On a fresh-run command, archive the existing file as `overnight-candidates-YYYY-MM-DD-archived.json` and rebuild from scratch.

##### Step 1 onward — company-by-company search work (only runs if Step 0 decided to add new candidates)

For each company Brian named (or Auto Mode selected) in Step 0's gap-fill:
...
Then **pre-schedule identical batches at 2-hour intervals** across the overnight window. Use `mcp__scheduled-tasks__create_scheduled_task` for each batch. Every batch fires with the same prompt: "read `overnight-candidates.json`, work the pending candidates from the top, run until 3 Yes-with-email outreach sequences fire OR the queue is exhausted, then end." Brian approves each `create_scheduled_task` call once at kickoff (session-level "always allow"), then walks away.

**Default batch schedule:** 7 batches at 7 PM / 9 PM / 11 PM / 1 AM / 3 AM / 5 AM / 7 AM = up to 21 sequences launched overnight. Brian can change batch count or spacing with an explicit override in the kickoff prompt.

**Why pre-schedule rather than self-spawn?** Tested 2026-04-17. An orchestrator scheduled task attempting to create child scheduled tasks in an unattended session failed silently because each `create_scheduled_task` call required a fresh tool approval Brian wasn't present to give. Pre-scheduling all batches at kickoff (while Brian is at the keyboard) sidesteps that constraint entirely.
...
1. **Cold-company selection.** Search HubSpot for companies owned by Brian (`hubspot_owner_id: 213536174`) with `notes_last_contacted` 6+ months ago or never contacted.
...
## TOOLS — owned by bc-prospect-qualification
...
## QUALIFICATION GATE — owned by bc-prospect-qualification

The verdict (Yes / No / Conditional) is formed by bc-prospect-qualification before this skill runs. If qualification did not return a Yes with email, this skill should never have been invoked. See the STOP IF NO EMAIL block at the top of this file.
...
### Voice and tone
- Tone: peer-to-peer, not vendor-to-buyer. Brian reaches out as Brian, not as a company.
- Emails are short. Mobile-friendly. Scannable in 10 seconds.
- Keep prose tight and direct. No fluff.

### Sign-off
- Do NOT type "Brian" or any name at the bottom of the email body. Outlook's signature block handles the sign-off automatically. Typing a name manually creates a doubled sign-off when Outlook appends the signature.
...
## Step 1: HubSpot ownership + contact lookup — owned by bc-prospect-qualification

Qualification performs the HubSpot ownership check (JAM only: Brian 213536174, Mark 210187184, John 210187193) and the contact-exists check. Do not repeat here. If the prospect reached this skill, qualification already cleared ownership.

---

## Step 2: ZoomInfo enrichment — owned by bc-prospect-qualification
...
No call tasks are created. The LOB is noted at the top of the strategy note so Brian can enroll the contact in the correct calls sequence manually.
...
Short. 3-4 sentences max. Lead with their specific pain based on role and company. Reference the Personal Hook. One clear ask at the end. No corporate speak. No "Brian" at the bottom — Outlook signature handles it.
...
> On [Day 1 date], Brian Charrette wrote:
> [Full Email 1 text]
...
## Step 6: Call script, voicemail, and LinkedIn invite — owned by bc-prospect-qualification
...
If the strategy note is missing these pieces, qualification was skipped or failed. STOP and invoke bc-prospect-qualification first.
...
Stop. Do not open Outlook until Brian says ready.
...
7. Do NOT click Send — leave it pre-composed for Brian
...
This is the ONLY HubSpot write this skill performs. All contact creation, strategy note writing, and LINKED_IN_CONNECT task CREATION are owned by bc-prospect-qualification and have already happened before this skill runs.
...
**Why this matters:** Brian's workflow is synchronized. LinkedIn invite sent at 2 PM, call sequence enrollment, voicemail, then Email 1 auto-fires at 4 PM. All four touches must land on the same day. If the LINKED_IN_CONNECT due_date and Email 1's Day 1 drift apart, the coordinated attack breaks.

**Everything else related to HubSpot (contact record, strategy note, LinkedIn invite text, LinkedIn message fallback tasks for no-email cases) is qualification's job.** If any of those are missing when this skill runs, STOP and invoke bc-prospect-qualification first.
...
- HubSpot Status: "Brian — HubSpot ID [id]" / "Not found"
...
After every overnight run, update Tab 2 for all 4 companies. Even companies that were not started get a row with status "Not Started" so Brian knows to carry them into the next run.
