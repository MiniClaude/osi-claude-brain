---
name: bc-monthly-account-count
description: >
  Track Brian Charrette's HubSpot account ownership over time — how many companies he owns,
  ...
  ALWAYS use this skill when Brian says "run account count", "BC monthly account count",
  "account count report", "how many accounts do I have", "show me my account changes",
  "what changed in my accounts", "account tracker", or any variation of wanting to see
  account ownership growth or changes over time.
---

# BC Monthly Account Count

Tracks Brian's HubSpot company ownership over time — total count, net new adds, reassignments in,
and removals — and renders an interactive HTML dashboard with a running chart.

## Key Constants

- **Brian's HubSpot Owner ID**: `213536174`
- **Skill base directory**: wherever this SKILL.md lives (e.g. `.claude/skills/bc-monthly-account-count/`)
- **Snapshots directory**: `snapshots/` (sibling to SKILL.md)
- **Output file**: `BC_Account_Count_YYYY-MM-DD.html` saved to Brian's workspace folder
...
date range ("since March", "since 4/1"), note it — you'll use that date as the comparison point
...
**Pagination is critical** — Brian owns 1,200+ companies. Keep calling with the `after` cursor
...
`BC_Account_Count_YYYY-MM-DD.html`.

#### Dashboard Structure

```
┌─────────────────────────────────────────────────────────┐
│  BC Monthly Account Count — April 20, 2026              │
...
[View BC_Account_Count_2026-04-20.html](computer://...path...)
