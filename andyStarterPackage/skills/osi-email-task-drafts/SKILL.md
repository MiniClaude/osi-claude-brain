...
**Included:**
- Andy's email tasks (ownerId `<<YOUR_HUBSPOT_OWNER_ID>>`) that are `NOT_STARTED` with `hs_timestamp <= end of today` (due today OR overdue)
...
Andy's `ownerId` is `<<YOUR_HUBSPOT_OWNER_ID>>`. Today's date comes from `<env>`.
...
search_crm_objects(
  ...
  filterGroups: [{
    filters: [
      { propertyName: "hubspot_owner_id", operator: "EQ", value: "<<YOUR_HUBSPOT_OWNER_ID>>" },
...
> You are drafting email reply content for Andy McLean (<<YOUR_EMAIL>>) at OSI Global, IT hardware / maintenance / optics. Today is {today's date}.
>
