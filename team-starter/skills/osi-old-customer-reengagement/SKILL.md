...
5 emails. 28-day cadence. the user sends Email 1 himself. The rest schedule automatically.
...
- Do NOT type "the user" or any name at the bottom of email bodies. Outlook signature handles the sign-off.
...
**Email 1** = next business day after today (or the day the user says "ready").
...
Nothing else. No greeting. No sign-off. No additional text.
Quoted Email 1 below in standard reply format:
> On [Email 1 date], [MY_FULL_NAME] wrote:
> [Full Email 1 text]
...
No name at the bottom. Outlook signature handles it.
...
- **Pick one at random and commit to it.** Do not ask the user which to use. Claude chooses.
  Use actual randomness across runs so the same sequence does not always get the safe choice.
- Present all 5 to the user with the selected one clearly marked (bold + "SELECTED")

If the user explicitly tells you to swap a subject line after seeing the set, honor that.
...
2. If login screen appears, stop and notify the user
3. Click New mail
...
5. Voicemails: 15 seconds max. No phone number. End with my email address spelled audibly ("that's [MY_EMAIL_SPOKEN]"). Always present or future tense ("I'm sending" or "I'm about to send"). Never past tense.
