...
description: >
  ...
  any non-OOO reply (including shipment address confirmations), flags anything needing my
...
Check everything running, catch anything broken, flag anything the user needs to act on. Output a clean summary. Fast.
...
When the user says "resume [name]": flip those entries back to `"pending"` and adjust send dates if needed so the next send is at least 2 business days out from today.

When the user says "cancel [name]": change to `"canceled-reply"` and leave them.
...
This gives the user a heads-up on what's going out today so I can make his calls before 3 PM.
...
- Never modify tasks except to cancel on a confirmed hard bounce, auto-pause on a non-OOO reply, or my explicit instruction.
- Never flag out-of-office auto-replies as actionable. Never pause on OOO.
- Auto-pause (not cancel) on non-OOO replies. the user decides whether to resume or cancel after reviewing.
...
- Keep the summary under 30 lines if possible. the user reads this at 11 AM and needs to act on it fast.
