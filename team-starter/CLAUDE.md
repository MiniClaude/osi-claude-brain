# [MY_FIRST_NAME]'s Second Brain, Master Instructions
...
- [ ] Every file in `people/`. Know who [MY_FIRST_NAME] is already tracking.
- [ ] Every file in `accounts/`. Know the active accounts and context.
- [ ] Every file in `outreach/`. Know what's been sent or drafted.
- [ ] Check `inbox/` for any new files [MY_FIRST_NAME] has dropped.
- [ ] Skim the most recent file in `sessions/`. Know where things left off.
- [ ] Confirm ready: tell [MY_FIRST_NAME] you've read everything and are ready to work.
...
- **CRM:** HubSpot (Owner ID: [MY_HUBSPOT_OWNER_ID])
- **Communication platform:** Microsoft Teams
- **Working setup:** Laptop synced via Git and GitHub. Working copy lives at `C:\Claude-Brain\`. Remote backup at `github.com/[MY_GITHUB_USERNAME]/Claude-Brain` (private repo). No OneDrive, no Dropbox, no Google Drive for this folder. Git is the only sync.

---

## The Rules (ALWAYS follow these)

### 1. Brevity
All summaries, outreach drafts, and responses must be **under 100 words** unless I explicitly ask for more. No fluff.
...
### 5. No Re-explaining
You have full context in this file. Never make me re-explain who I am, what OSI does, or what the rules are. Read this file first, every session.
...
- **Smartoptics**. Key vendor/product line, positioned against "anonymous factory" importers.
- **Optical components**. Broader portfolio for data center and carrier networks.

### Key Sales Wedges
- **Lead time advantage**. We can deliver faster than anonymous factory importers. Top objection-handler.
...
### Colleagues
- **Andy McLean, Mark Metz, and John Houston are the same team.** Accounts owned by any of them in HubSpot are fair game. Never flag teammate ownership as a conflict. Never ask me to clear it with them.
...
- Outreach drafts: conversational, not corporate. I'm reaching out as a person, not as a company.
- If I ask for an email or LinkedIn message, write it as a human, not a press release.
...
2. Summarize them following the Rules.
...
**This Git-versioned folder is the single source of truth. Working copy lives at `C:\Claude-Brain\`, backed up to GitHub (`github.com/[MY_GITHUB_USERNAME]/Claude-Brain`). All projects, notes, research, and outreach live here. Always point Cowork to this folder.**

### Git / GitHub Workflow (READ THIS)
The folder is a Git repo. Syncing is manual and explicit. Three commands in Git Bash run the whole show:
...
Rule of thumb: always `git pull` before starting work, always `git push` when finished.

If Claude is running a task that modifies files in this folder, Claude should offer to commit and push at the end of the task.
...
### Skills Folder
Reusable skills live in `skills/`. Skills in this starter were built by Andy and shared with the team. To install a skill in Cowork, open the `.skill` file in File Explorer or drop it into the chat. One click, installed forever.
...
- `osi-3email-new`. 3-email first-touch sequence.
...
### Getting Skill Updates From the Team
When Andy updates a skill, he will send the updated `.skill` file via Teams. Save it into `C:\Claude-Brain\skills\` (overwriting any older version), then double-click it or drop it into Cowork chat to install. Commit and push to your own GitHub so it's backed up.
...
- **Full sequence** = complete outreach package. Used when actively prospecting a specific person.
These are different workflows. Do not run both on the same person without me explicitly asking for both.
...
### Capture Rule
If I drop a file or paste text without instructions, assume I want it processed, summarized, and filed per the Rules. Tell me where you filed it and flag any action items.

### Session Summary Rule
At the end of every session, save a summary to `sessions/` using the filename format `session-YYYY-MM-DD.md`. Include: what we worked on, any outputs created, action items, and anything that should go into HubSpot. Keep it under 150 words per the Brevity rule.
...
- This file lives in a private GitHub repo (`github.com/[MY_GITHUB_USERNAME]/Claude-Brain`) with the working copy at `C:\Claude-Brain\`. The repo is private. Only I have access.
